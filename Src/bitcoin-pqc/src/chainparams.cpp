 ...
 * PQC net (regtest)
 */
class CPQCParams : public CChainParams
{
public:
    explicit CPQCParams(const ArgsManager& args)
    {
        strNetworkID = CBaseChainParams::PQCNET;
        consensus.signet_blocks = false;
        consensus.signet_challenge.clear();
        consensus.nSubsidyHalvingInterval = 150;
        // 합의 과정 자체는 키와 무관 => BPQ에서도 그대로 256bit 사용
        consensus.BIP16Exception = uint256();
        ...
        consensus.nPowTargetTimespan = /*14 * 24 * 60 * 60*/60 * 10;                                                  // two weeks
        ...
        pchMessageStart[3] = 0x4e; // N (net)
        nDefaultPort = 18766;      // PQC net 포트 설정
        nPruneAfterHeight = 1000; // ??
        m_assumed_blockchain_size = 0;
        m_assumed_chain_state_size = 0;
...
void CPQCParams::UpdateActivationParametersFromArgs(const ArgsManager& args)
{
...
std::unique_ptr<const CChainParams> CreateChainParams(const ArgsManager& args, const std::string& chain)
{
    ...
    } else if (chain == CBaseChainParams::PQCNET) {
        return std::unique_ptr<CChainParams>(new CPQCParams(args));
    }
