...
CPrivKey CKey::GetPrivKey() const
{
    ...
    ret = crypto_sign_keypair_priv(seckey.data());
    assert(ret);
...
CPubKey CKey::GetPubKey() const
{
    ...
    int ret = crypto_sign_keypair_pub(pk, (uint8_t*)begin());
    assert(ret);
...
bool CKey::Sign(/* const uint256& hash,*/ const uint512& hash, std::vector<unsigned char>& vchSig, bool grind, uint32_t test_case) const
{
    if (!fValid)
        return false;
    ...
    unsigned char sig[CPubKey::SIGNATURE_SIZE];
    uint32_t counter = 0;
    ...
    int ret = crypto_sign(sig, (size_t*)CPubKey::SIGNATURE_SIZE, hash.begin(), 512, begin());
