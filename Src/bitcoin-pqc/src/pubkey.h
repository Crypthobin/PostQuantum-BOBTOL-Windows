...
public:
    ...
    //! Construct a public key using begin/end iterators to byte data.
    template <typename T>
    CBOBPubKey(const T pbegin, const T pend)
    {
        Set(pbegin, pend);
    }

    //! Construct a public key from a byte vector.
    explicit CBOBPubKey(Span<const uint8_t> _vch)
    {
        Set(_vch.begin(), _vch.end());
    }
    ...
    //! Comparator implementation.
    friend bool operator==(const CBOBPubKey& a, const CBOBPubKey& b)
    {
        return a.vch[0] == b.vch[0] &&
               memcmp(a.vch, b.vch, a.size()) == 0;
    }
...
struct CExtBOBPubKey {
    ...
    friend bool operator==(const CExtBOBPubKey& a, const CExtBOBPubKey& b)
    {
        return a.nDepth == b.nDepth &&
