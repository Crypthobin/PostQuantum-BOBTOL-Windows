...
{
    //if (vchPubKey.size() < CBOBPubKey::COMPRESSED_SIZE) {
    //    //  Non-canonical public key: too short
    //    return false;
    //}
    //if (vchPubKey[0] == 0x04) {
    //    if (vchPubKey.size() != CBOBPubKey::SIZE) {
...
bool static IsValidSignatureEncoding(const std::vector<unsigned char>& sig)
{
    ...
    //if (sig.size() != 72 && sig.size() != 2421) return false;
    if (sig.size() != 2427) return false;
...
bool static IsLowDERSignature(const valtype& vchSig, ScriptError* serror)
{
    ...
    /*if (!CBOBPubKey::CheckLowS(vchSigCopy)) {
...
static bool EvalChecksigPreTapscript(const valtype& vchSig, const valtype& vchPubKey, CScript::const_iterator pbegincodehash, CScript::const_iterator pend, unsigned int flags, const BaseSignatureChecker& checker, SigVersion sigversion, ScriptError* serror, bool& fSuccess)
{
    ...
    //    return false;
    //}
    fSuccess = checker.CheckSignature(vchSig, vchPubKey, scriptCode, sigversion);
...
static bool EvalChecksig(const valtype& sig, const valtype& pubkey, CScript::const_iterator pbegincodehash, CScript::const_iterator pend, ScriptExecutionData& execdata, unsigned int flags, const BaseSignatureChecker& checker, SigVersion sigversion, ScriptError* serror, bool& success)
{
    ...
    printf("SigVersion is not WITNESS_V0!!!!!");
            ...
            if (fExec && 0 <= opcode && opcode <= OP_PUSHDATA4) {
                ...
                case OP_CHECKMULTISIGVERIFY: {
                    ...
                    while (fSuccess && nSigsCount > 0) {
                        ...
                        // Check signature
                        bool fOk = checker.CheckSignature(vchSig, vchPubKey, scriptCode, sigversion);
...
template <class T>
bool GenericTransactionSignatureChecker<T>::CheckSignature(const std::vector<unsigned char>& vchSigIn, const std::vector<unsigned char>& vchPubKey, const CScript& scriptCode, SigVersion sigversion) const
{
