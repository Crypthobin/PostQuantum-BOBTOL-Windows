...
const std::string CBaseChainParams::PQCNET = "pqcnet"; // 추가
...
void SetupChainParamsBaseOptions(ArgsManager& argsman)
{
    ...
    // 이부분 보완 필요
    argsman.AddArg("-pqcnet", "regtest 복붙함."
                              "argument 관련 옵션의 의미를 좀 더 공부해야 할 듯",
                   ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::CHAINPARAMS);
}
...
std::unique_ptr<CBaseChainParams> CreateBaseChainParams(const std::string& chain)
{
    if (chain == CBaseChainParams::MAIN) {
        ...
        return std::make_unique<CBaseChainParams>("pqcnet", 18765, 18767);
    } // 추가, port=18766로 시작
