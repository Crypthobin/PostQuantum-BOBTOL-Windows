...
uint512 SHA512Uint512(const uint512& input)
{
    uint512 result;
    CSHA512().Write(input.begin(), 32).Finalize(result.begin());
    return result;
}

CPQHashWriter TaggedHash512(const std::string& tag)
{
    CPQHashWriter writer(SER_GETHASH, 0);
    uint512 taghash;
    CSHA512().Write((const unsigned char*)tag.data(), tag.size()).Finalize(taghash.begin());
    writer << taghash << taghash;
    return writer;
}
